...
(function () {
  ...
  function _injectUserChip(user) {
    ...
    const _inject = () => {
      ...
      chip.innerHTML = `
        <div class="user-avatar" title="${fullName}" style="${isAdmin ? 'background:linear-gradient(135deg,#f59e0b,#d97706);' : ''}">${initials}</div>
        <div class="user-chip-info">
          <div class="user-chip-name">Hi, ${user.first_name}! ${isAdmin ? '<span class="admin-badge" style="margin-left:4px;font-size:8px;">ADMIN</span>' : ''}</div>
      ...
      // Always append as the last element so it sits at top-right
