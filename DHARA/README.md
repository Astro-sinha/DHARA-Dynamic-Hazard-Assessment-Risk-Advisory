...
8. **Field Reporting System**:
   - Citizen/officer photo upload with EXIF geotag extraction.
   - Reports persisted to `field_reports.json` via Flask REST API.
...
├── dhara_users.db        # SQLite database storing users (unique username, hashed pw)
...
├── flask_client.js       # Flask ML API client (auto-connects, graceful offline fallback)
├── server.py             # Flask REST API backend + Auth endpoints
├── requirements.txt      # Python dependencies
├── field_reports.json    # Auto-created by Flask for persisting field reports
...
| `GET`  | `/api/field-reports` | List all submitted field reports |
| `POST` | `/api/field-reports` | Submit a new field report |
